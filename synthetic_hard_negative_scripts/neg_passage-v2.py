...
if clean_corpus==True:
    corpus = dict(list(corpus.items())[:8_841_423])
    qrels = dict(qrels)
    for q_id, data in qrels.items():
        qrels[q_id] = dict([(p_id, score) for p_id, score in data.items() if score > 0])

    qrels = list(qrels.items())

### System instruction prompt ###
system_prompt='''You are a subject matter expert in your field with substantial accumulated
knowledge in a specific subject or topic, validated by academic degrees,
certifications, and/or years of professional experience in that field.

You will be given a question and passage(s). Your task is to write a new
passage that does not answer the question. However, this new passage
that you will create is to be related to the themes of the question and
passage(s).
...
old_dir = f"/work/mbouthil/datasets/msmarco_{old_dataset_name}"
modified_dir = f"/work/mbouthil/datasets/msmarco_{new_dataset_name}"
os.makedirs(modified_dir, exist_ok=True)
os.makedirs(os.path.join(modified_dir, "qrels"), exist_ok=True)

# Copying queries
shutil.copy(
    f"{old_dir}/queries.jsonl", 
