...
### Parameters ###
batch_size=256
tau_list=[0.01, 0.03, 0.05, 0.07, 0.1]
lr_list=[1e-6, 2e-6, 1e-5, 2e-5, 1e-4]
steps=10_000
...
fig, axes = plt.subplots(len(tau_list), len(lr_list), figsize=(24, 12))
x = range(1, steps+1)

k = 0
for i in range(len(tau_list)):
    for j in range(len(lr_list)):
        axes[i, j].plot(x, model_losses[k])
        axes[i, j].set_title(f"Tau={tau_list[i]}, lr={lr_list[j]}")
        
        axes[i, j].text(
            0.98, 0.98,
            f"Avg last 5000: {avg_loss[k]:.4f}",
...
plt.tight_layout()
plt.style.use('bmh')
plt.savefig(f"/work/mbouthil/MMATH-CM-Research-Project/RAG/figures/hypertune_tests.png", dpi=300)
