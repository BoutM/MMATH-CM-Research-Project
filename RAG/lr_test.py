...
batch_size=256
tau=0.03
learning_rates=[1e-6, 2e-6, 1e-5, 2e-5, 1e-4, 2e-4]
steps=15000
...
for lr in learning_rates:
    ...
    print(f"/ntau={tau} model training in progress...")
    ...
    print(f"tau={tau} model training complete")

### Plotting Model Losses ###

fig, axes = plt.subplots(2, int(len(learning_rates)/2), figsize=(24, 12))
x = range(1, steps+1)
axes = axes.flatten()
for i, loss in enumerate(model_losses):
    axes[i].plot(x, loss)
    axes[i].set_title(f"Model: LR={learning_rates[i]}")
    axes[i].text(
        0.98, 0.98,
        f"Average loss in last 5000 steps: {avg_loss[i]:.4f}",
...
plt.tight_layout()
plt.style.use('bmh')
plt.savefig(f"/work/mbouthil/MMATH-CM-Research-Project/RAG/figures/loss_curve_lr_tests.png", dpi=300)